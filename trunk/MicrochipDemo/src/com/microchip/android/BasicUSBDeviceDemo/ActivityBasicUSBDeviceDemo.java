...
/**
 * 
 * The main class the implements the GUI for the USB device demos
 * 
 */
public class ActivityBasicUSBDeviceDemo extends Activity {
	...
	/**
	 * loadDemo - tries to load the demo for the specified USB device, including
	 * changing the view and title.
	 * 
	 * @param device
	 *            The USB device whos demo to run
	...
	public Demo loadDemo(UsbDevice device) {
		...
		Demo tempDemo = Demo.loadDemo(this.getApplicationContext(), device,
				handler);
		...
		if (tempDemo != null) {
			...
			if ((tempDemo.getClass().equals(DemoLibUSB.class) == true)
				...
				findViewById(R.id.layout_toggle_led)
						.setVisibility(View.VISIBLE);
				findViewById(R.id.layout_button_status).setVisibility(
						View.VISIBLE);
				findViewById(R.id.layout_potentiometer_status).setVisibility(
						View.GONE);

			} else if (tempDemo.getClass().equals(DemoMCHPUSB.class)) {
				findViewById(R.id.layout_toggle_led)
						.setVisibility(View.VISIBLE);
				findViewById(R.id.layout_button_status)
						.setVisibility(View.GONE);
				findViewById(R.id.layout_potentiometer_status).setVisibility(
						View.VISIBLE);

			} else if (tempDemo.getClass().equals(DemoCustomHID1.class)) {
				findViewById(R.id.layout_toggle_led)
						.setVisibility(View.VISIBLE);
				findViewById(R.id.layout_button_status).setVisibility(
						View.VISIBLE);
	...
	/**
	 * Sets the R.id.title object of the current view with the specified String
	 * value.
	 * 
	 * @param s
	 *            The new title for the current view.
	 */
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.obj.getClass().equals(MessageButton.class)) {
				updateButton(R.id.button_status,
						((MessageButton) msg.obj).isPressed);
			} else if (msg.obj.getClass().equals(MessagePotentiometer.class)) {
				updatePotentiometer(((MessagePotentiometer) msg.obj).pressure,
						((MessagePotentiometer) msg.obj).percentageChar);
	 ...
	 * 
	 * @param id
	 *            The resource id of the TextView that needs to be changed.
	 * @param pressed
	...
	private void updateButton(int id, boolean pressed) {
		...
		/*
		 * Get the requested TextView and its parent layout using the passed in
		 * id.
		 */
		...
		/*
		 * Set the background resource to 0 to clear out the existing background
		 * information (removes artifacts).
		 */
	...
	private void updatePotentiometer(double pressure, String percentageChar) {
		TextView tv;
		tv = (TextView) findViewById(R.id.textView1);
		tv.setText("" + pressure);
