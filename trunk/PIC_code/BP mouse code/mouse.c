...
//ROM signed char dir_table[]={-4,-4,-4, 0, 4, 4, 4, 0};		//-10,-10,-10, 0, 10, 10, 10, 0
...
// ************** OUR CODE ***********************************************************************************************************************
	...
	if (PIR1bits.ADIF == 1)	{		PIR1bits.ADIF = 0;
		...
		//PORTBbits.RB6 = !PORTBbits.RB6;		//for frequency testing

		//copy over the data to the HID buffer
		buffer[0] = 0;
		buffer[1] = ADRESH;			//ADRESH+1;			//horizontal (X vector) displacement+1 to cover usb issues with values 0 and 256
		buffer[2] = ADRESL;	    		//ADRESL;			//vertical (Y vector) displacement
		if (ADRESH == 0 && ADRESL == 128)									//to cover usb issues with value 128
		{
			buffer[1] = 20;									//in matlab identifies the number 128
			buffer[2] = 100;
		}
		if (ADRESH != 0 && ADRESL == 128)									//to cover usb issues with value 128
		{
			buffer[1] = 20 + ADRESH;									//in matlab identifies the number 128
			buffer[2] = 100;
		}
...
/*		while(TXSTAbits.TRMT == 0);		// wait here till transmit complete
		TXREG = 0;					// send x to identify x displacement

		while(TXSTAbits.TRMT == 0);		// TRMT (Transmit Shift Register status bit) = 0 -> TSR full: Not ready to receive more tx data						
		TXREG = ADRESH;				// send horizontal (X vector) displacement

		while(TXSTAbits.TRMT == 0);		// TRMT = 0 -> TSR full:Not ready to receive more tx data
		TXREG = 'y';					// send y to identify y displacement

		while(TXSTAbits.TRMT == 0);		// TRMT = 0 -> TSR full: Not ready to receive more tx data
		TXREG = ADRESL;				// send vertical (Y vector) displacement		(BYTE)(data>>8);	//tx msb data -> send byte + significant
*/
	    }
...
static void InitializeSystem(void)
{
        ...
        ADCON1 = 0x00;      						// ADC: For Positive and Negative voltage reference supplied externally through VREF+/- pin.
		...
		INTCONbits.TMR0IE = 1;
		INTCONbits.TMR0IF = 0;
		INTCON2bits.TMR0IP = 1;						//TMR0IP: TMR0 Overflow Interrupt Priority bit: 1 = High priority

    	//Initialize all of the mouse data to 0,0,0 (no movement)
    	buffer[0]=buffer[1]=buffer[2]=0;

		//initialise USART:
 /*   	TXSTAbits.SYNC = 0;							//assyncronous mode
    	RCSTAbits.SPEN = 1;							//serial port enable: RB5/RX/DT and RB7/TX/CK pins as serial port pins
    	TXSTAbits.BRGH = 0;							//high speed mode
    	BAUDCONbits.BRG16 = 0;						//8-bit baud rate generator is used (SPBRG only)
    	SPBRG = 77; 								//155 -> Baud rate: 19.2kbps (Fosc = 48 MHz(12MHz x 4xPLL) clock) 0.16% error

    	TXSTAbits.TX9 = 0;							//8 bit transmission mode
    	TXSTAbits.TXEN = 1;							//enables transmission
    	RCSTAbits.RX9 = 0;							//8 bit reception mode
    	RCSTAbits.CREN = 0;							//disables reception
*/		
/*		do
		{
			Delay10KTCYx (0);
			while(TXSTAbits.TRMT == 0);		// wait here till transmit complete
			TXREG = 'x';					// send x to identify x displacement
		}
		while(1);
*/
...
//	to it in HardwareProfile.h.
    ...
    //UserInit();		//WARNING UNCOMMENT THIS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
{   
    ...
    //Emulate_Mouse();			//WARNINNNNNNNGGGGGGGGGGGGGGGG = ENABLE THIS!!!!!!!!!!!!!!!!!!!!!!!!11
