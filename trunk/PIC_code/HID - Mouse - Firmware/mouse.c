...
// ************** OUR CODE *************************	
	...
	if (PIR1bits.ADIF == 1)	{		PIR1bits.ADIF = 0;
/*
			if (vector == 255 || (vector == 0 && auxvar2 == 1))
				auxvar = !auxvar;
...
*/
		...
		buffer[0] = 0;
		buffer[1] = ADRESH;			//ADRESH;			//0xFF (-127)	0x7F (+127)									//horizontal (X vector) displacement
		buffer[2] = ADRESL;			//200*(256/1024) - 127;										//vertical (Y vector) displacement
...
// ************** END OF OUR CODE *********************
		...
		//Etc.
	//	INTCONbits.GIE = 1; //Re-enable Global Int bit. is it necessary???
...
static void InitializeSystem(void)
{
		...
		TRISCbits.TRISC6 = 1;
		ANSELHbits.ANS8 = 1; 	//Defining RC^ as Analog Input
        ADCON1 = 0x00;      // VDD and VSS for ADC
		ADCON0 = 0x21; 		//MODIFIED BY US
		ADCON2 = 0xB2;		//AA;		//Right justified, 20 Tad, FOSC/2  90
